        ...
        <div className="mt-2 mb-4 p-3 rounded-lg border bg-gradient-to-r from-slate-50 to-white dark:from-slate-900 dark:to-slate-950">
          ...
          <div className="grid grid-cols-5 gap-x-2 gap-y-1.5">
            {[
              ...
              { label: "Dashboard", cls: "bg-violet-100 text-violet-700 border-violet-200", tabs: "11-13" },
              ...
              <span key={g.label} className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-semibold border ${g.cls}`}>
                <span className="w-1.5 h-1.5 rounded-full bg-current opacity-60 shrink-0" />
                <span className="truncate">{g.label}</span> <span className="opacity-60 font-normal shrink-0">({g.tabs})</span>
              </span>
            ))}
          </div>
        </div>
